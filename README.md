...
- **Live sensors** — CPU and GPU temperature, both fan speeds, GPU draw and
  ...
  Hybrid and Eco never call `nvidia-smi`: their monitoring is a passive sysfs
  state check, leaving an idle dGPU free to sleep. Live NVIDIA temperature,
  power and utilisation are queried only in Ultimate mode, where the dGPU is
  already the display GPU. The same readings appear in the bar icon's tooltip,
  so "how hot is it right now" needs no click.
